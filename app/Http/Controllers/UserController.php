...
class UserController extends Controller
{
    ...
    {
        try {
            // Step 1: ค้นหา user จาก email ในตาราง users
            $user = DB::table('users')
                ->where('email', $request->username)
                ->first();

            // Step 2: ถ้าไม่เจอ email นี้ในระบบ
            if (!$user) {
                return response()->json([
                    'success' => false,
                    'message' => 'ไม่พบอีเมล "' . $request->username . '" ในระบบ'
                ]);
            }

            // Step 3: ตรวจสอบ password ผ่าน hash
            ...
            // เพิกถอน Token เก่าทั้งหมดของบัญชีนี้ (Logout จากเครื่องอื่นๆ)
            DB::table('tokens')
            ...
            // คำนวณวันหมดอายุ Token จาก Config
            $expiresAt = \App\Utils\JwtUtil::parseTTL(config('jwt.token_ttl'));

            // สร้าง Payload ตัวนี้จะถูกเข้ารหัสเข้าไปใน JWT
            $payload = [
            ...
            // สร้าง String ของ Token กลับมา (เข้ารหัส HMAC SHA256)
            $tokenString = \App\Utils\JwtUtil::createToken($payload);
            ...
            // บันทึกลง Session ให้ระบบดึงไปเช็คใน Middleware
            Session::put('token', $tokenString);
